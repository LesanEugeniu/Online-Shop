...
<body>
    ...
    <div class="container-1">
        ...
        <div class="wrapper">
            ...
            $qry = mysqli_query($conn, "SELECT id FROM users WHERE unique_id = '{$unique_id}'");
            $row = mysqli_fetch_assoc($qry);
            $id = $row['id'];
            ...
            $queryCount = mysqli_query($conn, "SELECT COUNT(orders_id) FROM orders");
            ...
            if ($rowCount[0] != 0) {
                ...
                echo "
                    <tr class=\"table-header\">
                        <th class=\"header__item\">Product Image</th>
                        <th class=\"header__item\">Product Name</th>
                        <th class=\"header__item\">Quantity</th>
                        <th class=\"header__item\">Buyer First Name</th>
                        <th class=\"header__item\">Buyer Last Name</th>
                        <th class=\"header__item\">Buyer Email</th>
            ...
            while ($row = mysqli_fetch_assoc($query)) {
                ...
                $queryUser = mysqli_query($conn, "SELECT firstName, lastName, email FROM users WHERE id = '{$user_id}'");
                $rowUser = mysqli_fetch_assoc($queryUser);
                $queryProduct = mysqli_query($conn, "SELECT brand, model_name, image FROM product WHERE product_id = '{$product_id}'");
                ...
                if ($product_id <= 25) {
                    echo "
                        <td class=\"table-data\"><img class=\"images\" src=\"" . substr($rowProduct["image"], 1) . "\"></td>
                        ";
                } else {
                    echo "
                        <td class=\"table-data\"><img class=\"images\" src=\"./ProductsImages/" . $rowProduct["image"] . "\"></td>
                        ";
                }
                echo "
                    <td class=\"table-data\">" . $rowProduct['brand'] . " " . $rowProduct['model_name'] . "</td>
                    <td class=\"table-data\">" . $row['quantity'] . "</td>
                    <td class=\"table-data\">" . $rowUser['firstName'] . "</td>
                    <td class=\"table-data\">" . $rowUser['lastName'] . "</td>
                    <td class=\"table-data\">" . $rowUser['email'] . "</td>
                    <td class=\"table-data\">
                    <a href=\"php/deleteOrder.php?product_id=$product_id&user_id=$user_id\">
